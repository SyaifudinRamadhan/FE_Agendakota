...
const reschedule = async ({ visitDate, seatNumber, pchId }) => {
	try {
...
const invite = async ({ targetEmail, pchId }) => {
	try {
		let res = await axios.post(
...
const getBackInvite = async ({ pchId }) => {
	try {
		let res = await axios.post(
...
const downloadTicket = async ({ pchId }) => {
	try {
		let res = await axios.get(
			process.env.REACT_APP_BACKEND_URL +
...
const PopUpTicketIn = ({
	...
	const handleInvitation = (pchId) => {
		...
		if (
			...
			invite({ targetEmail: emailDest.current.value, pchId }).then((res) => {
				if (res.status === 201) {
					trx.purchases.forEach((pch) => {
						if (pch.id === pchId) {
							pch.is_mine = 0;
	...
	// 		if (pausedProcess === "reschedule") {
	// 			handleReschedule(ticketData, visitDate, seatNumber);
	// 		} else if (pausedProcess === "invite") {
	// 			handleInvitation(ticketData.id);
	// 		} else if (pausedProcess === "refund") {
	...
	// useEffect(() => {
	// 	// 1. Re-Set seatNumber (check if default selected is available when user change visit date ?)
	// 	if(new Date(ticketData.visit_date.visit_date).setHours(0,0,0,0) != new Date(visitDate.format()).setHours(0,0,0,0)){
	...
	useEffect(() => {
		if (trx) {
			...
			let start = new Date(
				trx.purchases[0].ticket.event.start_date +
					" " +
					trx.purchases[0].ticket.event.start_time
			);
			let end = new Date(
				trx.purchases[0].ticket.event.end_date +
					" " +
					trx.purchases[0].ticket.event.end_time
			);
			setStart(
			...
			let pchsGroup = [];
			trx.purchases.forEach((pch, index) => {
				if (
					!pchsGroup[
						`${pch.ticket.id}-${
