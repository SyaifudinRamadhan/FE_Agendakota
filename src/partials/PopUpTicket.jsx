...
const deleteTicketEnd = async ({ orgId, eventId, ticket_id }) => {
	try {
		let res = await axios.post(
			process.env.REACT_APP_BACKEND_URL +
				"/api/org/" +
			...
			updateTicketEnd({
				...
				if (res.status === 202) {
					...
					tickets[selectedTicketIndex].price =
						defaultTypePrice == "1" ? 0 : price.current.value;
	...
	const handleSubmitTicket = () => {
		if (
			...
			setAlert({
				state: true,
				content:
					defaultTypePrice != "1" && price.current.value < 10000
											...
											<>
												<div className={styles2.DateGroup}>
													...
													<InputLabeled
														...
														iconSvg={<BiCalendar />}
														label={
															<p className={styles2.TextSecondary}>Berakhir</p>
														}
														style={{ boxShadow: "none", outline: "none" }}
														refData={end}
													/>
