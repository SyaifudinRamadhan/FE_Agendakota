...
const addAcc = async ({ orgId, bankName, accNumber, accName }) => {
	try {
...
const deleteAcc = async ({ orgId, accId }) => {
	try {
		let res = await axios.post(
...
const verifyAcc = async ({ orgId, bankAccId, otpCode }) => {
	try {
		let res = await axios.post(
...
const detailWd = async ({ orgId, wdId }) => {
	try {
		let res = await axios.get(
			process.env.REACT_APP_BACKEND_URL +
				"/api/org/" +
...
const addWd = async ({ orgId, eventId, bankAccId }) => {
	try {
		let res = await axios.post(
			process.env.REACT_APP_BACKEND_URL +
				"/api/org/" +
		...
		return detailWd({ orgId: orgId, wdId: res.data.data.id });
	} catch (error) {
		return handleError(error);
	}
};

const deleteWd = async ({ orgId, wdId }) => {
	try {
		let res = await axios.post(
			process.env.REACT_APP_BACKEND_URL +
				"/api/org/" +
...
const OrganizerBilling = ({ organization, fnSetLogin, isLogin }) => {
	...
	const handleDeleteProcess = (idBank) => {
		setLoadingPopUp(true);
		// calling backend
		deleteAcc({ orgId: orgSelected, accId: idBank }).then((res) => {
			if (res.status === 202) {
	...
	const handleDeleteProcessWd = (wdId) => {
		setLoadingPopUp(true);
		// calling backend
		deleteWd({ orgId: orgSelected, wdId: wdId }).then((res) => {
			if (res.status === 202) {
	...
	useEffect(() => {
		if (firstLoad === false) {
			loadEvents({ orgId: organization[0].id }).then((res) => {
				if (res.status === 200) {
			...
			loadAvlWd({ orgId: organization[0].id }).then((res) => {
				if (res.status === 200) {
					setAvlWds(res.data);
				} else if (res.status === 401) {
					fnSetLogin(false);
	...
	useEffect(() => {
		...
		if (organization.length === 0) {
			...
			loadBanks({ orgId: organization[0].id }).then((res) => {
				if (res.status === 200) {
			...
			loadAccs({ orgId: organization[0].id }).then((res) => {
				if (res.status === 200) {
			...
			loadWds({ orgId: organization[0].id }).then((res) => {
				if (res.status === 200) {
