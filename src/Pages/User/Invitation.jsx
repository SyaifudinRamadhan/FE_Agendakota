...
const loadInvitationsRcv = async () => {
	try {
...
const loadInvitationsSdr = async () => {
	try {
		let res = await axios.get(
...
const Invitation = ({ isLogin, fnSetLogin = () => {} }) => {
	...
	const handleReceiveInv = (invId) => {
		setLoading(true);
		acceptInvitation({ invitationId: invId }).then((res) =>
			handleResponsePorcess(res, invId, true)
		);
	};

	const handleCancelInv = (invId) => {
		setLoading(true);
		rejectInvitation({ invitationId: invId }).then((res) =>
			handleResponsePorcess(res, invId, false)
		);
	};
